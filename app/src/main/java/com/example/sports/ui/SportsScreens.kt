...
fun SportsApp(
    ...
    val contentSize: SportsContentType = when(windowSize) {
    ...
    Scaffold(
        ...
        if(contentSize == SportsContentType.LIST_AND_DETAIL) {
            SportsListAndDetails(
                sportsUiState = uiState,
                onSportSelected = { viewModel.updateCurrentSport(it)},
...
fun SportsAppBar(
    ...
    TopAppBar(
        ...
        navigationIcon = if (
            contentSize != SportsContentType.LIST_AND_DETAIL && !isShowingListPage
        ) {
            {
